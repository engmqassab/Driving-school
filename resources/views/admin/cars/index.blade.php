            ...
            <div class="card">
                ...
                <div class="card-body pt-5">
                    <div class="table-responsive">
                        <!--begin::Table-->
                        <table class="table align-middle table-row-dashed cars-list-table fs-6 gy-5">
                            <!--begin::Table head-->
                            <thead>
                            <!--begin::Table row-->
        ...
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal-title">{{__('أضافة مركبة')}}</h5>
                    <!--begin::Close-->
                    ...
                    <form id="modal_add_new_form" class="form" action="javascript:void(0)">
                        ...
                        <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_add_user_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_add_user_header" data-kt-scroll-wrappers="#kt_modal_add_user_scroll" data-kt-scroll-offset="300px">
                            ...
                            <div class="fv-row mb-7">
                                <!--begin::Label-->
                                <label class="required fw-bold fs-6 mb-2">{{__('الرقم')}}</label>
                                <!--end::Label-->
                                <!--begin::Input-->
                                <input type="text" name="car_number" id="car_number" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="الرقم"/>
                                <!--end::Input-->
                            </div>
                            <!--end::Input group-->
                            <!--begin::Input group-->
                            <div class="fv-row mb-7">
                                <!--begin::Label-->
                                <label class="required fw-bold fs-6 mb-2">{{__('سنه الصنع')}}</label>
                                <!--end::Label-->
                                <!--begin::Input-->
                                <input type="text" name="year" id="year" class="form-control form-control-lg form-control-solid pickadate-ar"  placeholder="YYYY-MM-DD"
                                       readonly="readonly"/>
                                <!--end::Input-->
                            </div>
                            <!--end::Input group-->
                            ...
                            <div class="fv-row mb-7">
                                <!--begin::Label-->
                                <label class="required fw-bold fs-6 mb-2">{{__('النوع')}}</label>
                                <!--end::Label-->
                                <!--begin::Input-->
                                <select name="type_id" id='type_id'data-control="select2" data-placeholder="اختر نوع" data-hide-search="true" class="form-select form-select-solid fw-bolder">
                                    <option></option>
                            ...
                            <div class="fv-row mb-7">
                                <!--begin::Label-->
                                <label class="required fw-bold fs-6 mb-2">{{__('الترخيص')}}</label>
                                <!--end::Label-->
                                <!--begin::Input-->
                                <select name="license_id" id="license_id" data-control="select2" data-placeholder="اختر ترخيص" data-hide-search="true" class="form-select form-select-solid fw-bolder">
                                    <option></option>
                                    @foreach ($car_licenses as $license)
                            ...
                            <div class="fv-row mb-7">
                                <!--begin::Label-->
                                <label class="required fw-bold fs-6 mb-2">{{__('التأمين')}}</label>
                                <!--end::Label-->
                                <!--begin::Input-->
                                <select name="insurance_id" id="insurance_id" data-control="select2" data-placeholder="اختر تأمين" data-hide-search="true" class="form-select form-select-solid fw-bolder">
                                    <option></option>
                                    @foreach ($car_insurances as $insurance)
...
@section('js')
    ...
    <script>
        var $url = "{{ route('admin.cars.index') }}";
        var $deleteAllurl = "{{ route('admin.cars-destroyAll') }}";
    ...
    <script>
        ...
        $(function() {
            ...
            $('#modal_create_new').on('hidden.bs.modal', function () {
                ...
                $('#add_new').html("حفظ");
                $('#modal-title').html('أضافة مركبة');
                $('#record_id').val('');
                $('#car_number').val('');
                $('#year').val('');
                $('#type_id').val('');
                $('#select2-type_id-container').html('اختر نوع');
                $('#license_id').val('');
                $('#select2-license_id-container').html('أختر ترخيص');
                $('#insurance_id').val('');
                $('#select2-insurance_id-container').html('أختر تأمين');
                $('#description').val('');
                ...
                if($(this).attr('id') == 'add_new'){
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('.cars-list-table').DataTable().ajax.reload();
                            $("#add_new").html("{{ __('حفظ') }}");
                        },
                        error: function(data) {
            ...
            $('body').on('click', '.editRecord', function() {
                ...
                $.get("{{ url('admin/cars') }}" + '/' + record_id + '/edit', function(data) {
                    $('#add_new').html("تعديل");
                    $('#modal-title').html('تعديل مركبة');
                    $('#modal_create_new').modal('show');
                    $('#record_id').val(data.id);
                    $('#car_number').val(data.car_number);
                    $('#year').val(data.year);
                    $('#type_id').val(data.type_id);
                    $('#select2-type_id-container').html(data.type);
                    $('#license_id').val(data.license_id);
                    $('#select2-license_id-container').html(data.license);
                    $('#insurance_id').val(data.insurance_id);
                    $('#select2-insurance_id-container').html(data.insurance);
                    $('#description').val('');
